...
export const useExpenses = () => {
  ...
  useEffect(() => {
    if (currentDepartment?.id) {
      dispatch(fetchSubDepartments(currentDepartment.id));
  ...
  useEffect(() => {
    const buildFilters = () => {
      ...
      if (debouncedSearch?.trim()) {
        filters.userName = debouncedSearch.trim();
      }

      if (filterMonth !== "" && filterMonth !== null) {
        filters.month = Number(filterMonth);
      }

      if (filterYear !== "" && filterYear !== null) {
        filters.year = Number(filterYear);
      }

      if (currentDepartment?.id) {
        filters.department = currentDepartment.id;
      }

      if (currentSubDepartment?.id) {
        filters.subDepartment = currentSubDepartment.id;
      }
    ...
    if (role === "superadmin") {
      if (hasFilters) {
        ...
        // 👇 ALWAYS fetch when filters are cleared
        dispatch(
          fetchExpenses({
            page,
            limit,
  ...
  const handleAdd = async () => {
    ...
    if (addExpense.fulfilled.match(response)) {
      await Promise.all([
        dispatch(
          fetchBudgets({ page: 1, limit: 10, month: "", year: "", all: false }),
        ),
  ...
  const handleSubmit = async () => {
    ...
    try {
      const resultAction = await dispatch(
        updateExpense({ id: selectedExpense.id, updates: formData }),
      );
