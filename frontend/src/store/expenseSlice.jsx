...
export const fetchExpenses = createAsyncThunk(
  ...
  async (
    { page = 1, limit = 20, location = "OVERALL" },
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASEURL}/expenses?${query}`,
        {
          method: "GET",
          credentials: "include",
...
export const fetchExpensesForUser = createAsyncThunk(
  ...
  async ({ userId, page = 1, limit = 20 }, { getState, rejectWithValue }) => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASEURL}/expenses/user/${userId}?${query}`,
        {
          method: "GET",
          credentials: "include",
          headers: { "Content-Type": "application/json", "x-csrf-token": csrf },
    ...
    try {
      ...
      const query = new URLSearchParams({
        ...
        ...(isReimbursed !== undefined && {
          isReimbursed: String(isReimbursed),
        }),
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASEURL}/expenses/search?${query}`,
        {
          method: "GET",
          credentials: "include",
          headers: { "Content-Type": "application/json", "x-csrf-token": csrf },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(searchExpenses.fulfilled, (state, action) => {
        ...
        // USER + ADMIN SEARCH RESULT
        state.expenses = action.payload.data;
        state.allExpenses = action.payload.allExpenses;

        // ❌ DO NOT CLEAR adminExpenses
        // ❌ DO NOT TOUCH adminMeta
