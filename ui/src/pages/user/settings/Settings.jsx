...
const Settings = () => {
  ...
  const [twoFactorStep, setTwoFactorStep] = useState("idle"); // idle, qr, verify, disable
  ...
  const handleSaveProfile = async (e) => {
    e.preventDefault();
    try {
      const res = await dispatch(updateUserProfile({ ...formData, userId: user.id })).unwrap();
      setIsEditing(false);
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8 px-4 md:px-8 space-y-8 animate-in fade-in duration-500">
      <PageHeader 
        title="Account"
        highlight="Settings"
        subtitle="Secure your account and manage your personal preferences."
      />
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="rounded-[20px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white border-0 overflow-hidden">
            ...
            <CardContent className="px-6 pb-8 -mt-12 text-center">
              ...
              <Badge className={cn(
                "px-4 py-1.5 rounded-full font-bold text-[10px] tracking-widest uppercase border-0 self-center",
                user?.role === "superadmin" ? "bg-amber-100 text-amber-700" : "bg-primary-50 text-primary-700"
              )}>
                {user?.role === "superadmin" ? "Global Admin" : "Standard User"}
              </Badge>
              ...
              <div className="mt-8 pt-8 border-t border-slate-100 space-y-4 text-left">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-slate-900 font-bold font-mono text-[10px]">{user?.id?.split('-')[0]}...</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500 font-medium">Location</span>
                  <span className="text-slate-900 font-bold">{user?.userLoc || "Overall"}</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-[20px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white border-0 p-6">
             <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Security Score</h4>
             <div className="space-y-4">
                <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                   <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: user?.twoFactorEnabled ? "90%" : "40%" }}
                    className={cn("h-full rounded-full", user?.twoFactorEnabled ? "bg-emerald-500" : "bg-amber-500")}
                   />
                </div>
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="font-semibold text-slate-700">
                    {user?.twoFactorEnabled ? "Highly Secure" : "Medium Security"}
                  </span>
                </div>
             </div>
          </Card>
                ...
                {isEditing ? (
                  ...
                  >
                    ...
                    <div className="md:col-span-2 pt-4 flex gap-3">
                       <Button type="submit" size="lg" className="rounded-xl px-8 bg-primary-600 hover:bg-primary-700 shadow-md shadow-primary-500/30">
                  ...
                  >
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-slate-900 font-bold">{user?.phone || "Private"}</p>
                      </div>
                    </div>
                  </motion.div>
                )}
              ...
              <div className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between p-6 rounded-2xl bg-slate-50/50 border border-slate-100 gap-4">
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-slate-900">Authenticator App (TOTP)</h4>
                        <p className="text-xs text-slate-500 font-medium">Use a mobile app to get security codes.</p>
                      </div>
                   </div>
                   <div className="flex items-center gap-4">
             ...
             >
                <div className="text-center space-y-4">
                   ...
                   <h3 className="text-xl font-bold text-slate-900">
                      {twoFactorStep === "qr" ? "Setup Authenticator" : 
                       twoFactorStep === "disable" ? "Disable Security?" : "Verify Identity"}
                   </h3>
                   <p className="text-slate-500 text-[11px] font-medium px-6 leading-relaxed">
                      {twoFactorStep === "qr" ? "Scan the QR code below with your preferred authenticator app." : 
                       twoFactorStep === "disable" ? "Please enter your 6-digit code to confirm deactivation." : 
                ...
                <div className="mt-8 space-y-6">
                   {twoFactorStep === "qr" && qrCodeData && (
                     <div className="flex flex-col items-center">
                        ...
                        <Button variant="link" size="sm" className="text-xs font-bold text-primary-600 flex items-center gap-1.5 hover:no-underline">
                           <Copy className="w-3.5 h-3.5" /> Copy Setup Key
                        </Button>
                     </div>
                   )}
                   ...
                   <div className="space-y-4">
                      ...
                      >
                         {twoFactorStep === "disable" ? "Deactivate Now" : "Complete Activation"}
                      </Button>
