...
export const fetchReimbursements = createAsyncThunk(
  ...
  async ({ location, page, limit, status } = {}, { getState, rejectWithValue }) => {
    try {
      const csrf = getState().auth.csrf;

      const params = new URLSearchParams();
      if (location && location !== 'undefined') params.append("location", location);
      if (page && page !== 'undefined') params.append("page", String(page));
      if (limit && limit !== 'undefined') params.append("limit", String(limit));
      if (status && status !== 'undefined') params.append("status", status);

      const res = await fetch(
        `${import.meta.env.VITE_API_BASEURL}/reimbursement?${params}`,
        {
          credentials: "include",
          headers: { 
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchReimbursements.rejected, (state, action) => {
        ...
        toast.error(action.payload || "Failed to fetch reimbursements");
      })
      ...
      .addCase(fetchReimbursementsForUser.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
        toast.error(action.payload || "Failed to fetch user reimbursements");
      })
      ...
      .addCase(markAsReimbursed.fulfilled, (state, action) => {
        ...
        const index = state.reimbursements.findIndex(
          (r) => r.id === updatedReimbursement.id,
        );
      ...
      .addCase(markAsReimbursed.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
        toast.error(action.payload || "Failed to update reimbursement");
      });
  },
});
